...
CONFIG_PATH = ROOT_DIR / "configs" / "experiment_roberta_transfer_matrix.yaml"
...
def save_macro_f1_matrix(grouped_df: pd.DataFrame, run_dir: Path) -> None:
    matrix_df = grouped_df.pivot(
        index="source_domain",
        columns="target_domain",
        values="macro_f1_mean",
    )
    matrix_df.to_csv(run_dir / "macro_f1_matrix.csv")
...
def main():
    ...
    save_macro_f1_matrix(grouped_df, parent_run_dir)
    ...
    summary = {
        ...
        "num_transfers": len(domains) * (len(domains) - 1),
